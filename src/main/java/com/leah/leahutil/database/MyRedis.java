package com.leah.leahutil.database;
...
//            readWriteLock.writeLock().unlock();
    ...
    public List<IPMessage> getIpsToList() {
            ...
            List<byte[]> bytes = jedis.lrange("ip-proxy-pool".getBytes(),0,-1);
            readWriteLock.readLock().unlock();
            return bytes.stream().map(o-> (IPMessage)SerializeUtil.unserialize(o)).collect(Collectors.toList());
    }
    public long delKeys(String key) {
    	 readWriteLock.writeLock().lock();
    	 long del = jedis.del(key.getBytes());
         readWriteLock.writeLock().unlock();
         return del;
    }
    // 将Redis中保存的对象进行反序列化
